...
- (void)reinitWithAttributeStride:(GLsizei)stride numberOfVertices:(GLsizei)count bytes:(const GLvoid *)dataPtr{
    NSCParameterAssert(0 < stride);
    NSCParameterAssert(0 < count);
    NSCParameterAssert(NULL != dataPtr);
    NSAssert(0 != self.vertexBufferID, @"invalid vertexBufferID");
    
    self.stride = stride;
    self.size = count * stride;
    glBindBuffer(self.vertexBufferID, GL_ARRAY_BUFFER);
    glBufferData(GL_ARRAY_BUFFER, self.size, dataPtr, GL_DYNAMIC_DRAW);
}
